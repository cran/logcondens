plot.dlc <- function(x, which = c("density", "log-density", "CDF"), add.title = TRUE, legend.pos = "topright", ...) {
  ...
  for (i in 1:length(xs)) {
    f.mat[i, 1:3] <- evaluateLogConDens(xs[i], object, which = 1:3)[, c("log-density", "density", "CDF")]
  }
  ...
  if (add.title == TRUE) {
    main.tit <- "Log-concave density estimation from i.i.d. data"
    o <- 4
  }
  ...
  if (which == "density") {
    ...
    if (object$smoothed == TRUE) {
      lines(object$xs, object$f.smoothed, col = 3, lwd = 2)
    }
    if (add.title == TRUE) {
      mtext("Density estimate. Dashed vertical lines indicate knots of the log-density.", 3, 0.5, cex = .75)
    }
  ...
  if (which == "log-density") {
    ...
    if (object$smoothed == TRUE) {
      lines(object$xs, log(object$f.smoothed), col = 3, lwd = 2)
    }
    if (add.title == TRUE) {
      mtext("Estimate of log-density. Dashed vertical lines indicate knots of the log-density.", 3, 0.5, cex = .75)
    }
  ...
  if (which == "CDF") {
    ...
    if (add.title == TRUE) {
      mtext("Estimate of CDF based on density.", 3, 0.5, cex = .75)
    }
