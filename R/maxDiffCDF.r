maxDiffCDF <- function(res1, res2, which = c("MLE", "smooth"), n.grid = 500) {
    ...
    if (r <= l) {
      ...
      if (length(i) > 0) {
        ...
        for (j in 1:length(tmp)) {
          x0 <- rbind(x0, c(tmp[j], abs(evaluateLogConDens(tmp[j], res1, which = 3)[, "CDF"] - evaluateLogConDens(tmp[j], res2, which = 3)[, "CDF"])))
        }
      }
  ...
  if ("smooth" %in% which) {
    ...
    test.stats <- locs
    zero.fun <- function(q, res1, res2) {
      return(evaluateLogConDens(q, res1, which = 4)[, "smooth.density"] - evaluateLogConDens(q, res2, which = 4)[, "smooth.density"])
    }
