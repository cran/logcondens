evaluateLogConDens <- function(xs, res, which = 1:5, gam = NULL, print = FALSE) {
  ...
  for (i in 1:length(xs)) {
    ...
    if (x0 < x[1]) {
      values[1:3] <- c(-Inf, 0, 0)
    }
    if (x0 == x[1]) {
      values[1:3] <- c(phi[1], exp(phi[1]), 0)
    }
    if (x0 > x[n]) {
      values[1:3] <- c(-Inf, 0, 1)
    }
    ...
    if (max(which) > 3) {
      qs <- Q00(x = x0, a = a, u = xj, v = xj1, gamma = gam, QFhat = (5 %in% which))
    }

    ## smooth PDF
    if (4 %in% which) {
      values[4] <- sum(f[js - 1] * qs$q)
    }

    ## smooth CDF
    if (5 %in% which) {
      values[5] <- sum(f[js - 1] * qs$Q)
    }

    values.mat[i, ] <- c(x0, values)

    i10 <- i / length(xs) * 10
    if ((round(i10) == i10) & (print == TRUE)) {
      print(paste(i10 * 10, "% of computation of smooth estimates done", sep = ""))
    }
  }
